...
const insertBaseConfigToHtml = (htmlStr, config) => {
  const newHtmlStr = Object.keys(config).reduce(
    (accumulator, key) =>
      accumulator.replace(
        new RegExp(`<!--baseConfig\\-${key}-->`, 'g'),
        config[key]
      ),
    htmlStr
  );
...
const template = fs.readFileSync(
  path.join(__dirname, './dist/index-origin.html'),
  'utf-8'
);
const initdata = fs.readFileSync(
  path.join(__dirname, './config/components.json'),
  'utf-8'
);
 ...
 * 预览页面 脚本激活
 */
const previewInsertedScriptStr = fs.readFileSync(
  path.join(__dirname, './preview-inserted-script.js'),
  'utf-8'
);
